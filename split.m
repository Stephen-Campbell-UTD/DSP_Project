...
dec_mono = downsample(mono,25);
dec_sub = downsample(sub,25);
Fs_mic = 400e3/25;
num_mic_samples = length(dec_mono);
...
subplot(2,1,1)
title('dec mono fft');
plot(freq_mic,mag2db(abs(fft(dec_mono))))
subplot(2,1,2)
title('dec sub fft')
plot(freq_mic, mag2db(abs(fft(dec_sub))))
